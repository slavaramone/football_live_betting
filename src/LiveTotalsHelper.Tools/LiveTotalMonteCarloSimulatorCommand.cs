...
public sealed class LiveTotalMonteCarloCommandOptions
{
    public string CurvesPath { get; init; } = "outputs/calibration/state-weibull-curves.json";
    public string SideModelPath { get; init; } = "outputs/calibration/next-goal-side-model.json";
    public string OutputPath { get; init; } = "outputs/debug/live-total-mc.json";
...
public sealed class LiveTotalMonteCarloCommandResult
{
...
public sealed class LiveTotalMonteCarloSimulatorCommand
{
    ...
    public async Task<LiveTotalMonteCarloCommandResult> RunAsync(
        LiveTotalMonteCarloCommandOptions options,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(options.CurvesPath))
            throw new ArgumentException("Curves JSON path is required.", nameof(options));
        ...
        if (string.IsNullOrWhiteSpace(options.SideModelPath))
            throw new ArgumentException("Next-goal-side model JSON path is required.", nameof(options));
        if (!File.Exists(options.SideModelPath))
        ...
        LiveMonteCarloSimulationResult simulation = simulator.Run(new LiveMonteCarloSimulationOptions
        {
            Request = request,
            Curves = curves,
        ...
        return new LiveTotalMonteCarloCommandResult
        {
