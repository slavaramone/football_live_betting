...
public static class TotalGoalsPricingCalculator
{
    ...
    public static double EstimateMarketTotalGoals(double line, double fairOverProbability)
    {
        ...
        // Empirical-only baseline: use market line as the total-goals anchor and apply a small
        // no-vig price tilt. This intentionally avoids parametric score distribution assumptions.
        return Math.Clamp(line + (fairOverProbability - 0.5) * 0.5, 0.05, 12.0);
    }
    ...
    public static IReadOnlyDictionary<int, double> BuildMeanDistribution(double mean, int maxGoals = 12)
    {
        mean = Math.Max(0.0, mean);
        maxGoals = Math.Max(1, maxGoals);

        int lower = (int)Math.Floor(Math.Min(mean, maxGoals));
        int upper = Math.Min(lower + 1, maxGoals);
        double upperWeight = Math.Clamp(mean - lower, 0.0, 1.0);

        var result = new Dictionary<int, double>();
        result[lower] = 1.0 - upperWeight;
        if (upper != lower)
            result[upper] = upperWeight;
